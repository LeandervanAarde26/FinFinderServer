  ...
  try {
    ...
    if (existingDecoration) {
      return res
        .status(409)
        .json({
          err: `Decoration with name ${Name} already exists in database.`,
        });
    }
    ...
    return res
      .status(200)
      .json({ msg: `New Decoration ${savedDecoration.name} added` });
  } catch (error) {
    console.log(error);
...
async function getById(req: Request, res: Response) {
  ...
  try {
    ...
    if (!individualDecoration) {
      return res
        .status(404)
        .json({ error: "Decoration was not found, please try again" });
    }
    return res
      .status(200)
      .json(
        individualDecoration ?? {
          error: `Decoration with ${id} was not found.`,
        }
      );
  } catch (error) {
    console.log(error);
