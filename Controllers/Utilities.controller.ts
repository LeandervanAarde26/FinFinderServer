...
async function addUtility(req: Request, res: Response) {
  try {
    ...
    if (existingUtility) {
      return res
        .status(409)
        .json({ err: `Utility with name ${Name} already exists in database.` });
    }
    ...
    return res
      .status(200)
      .json({ msg: `New Decoration ${savedUtility.name} added` });
  } catch (error) {
    console.log(error);
...
async function getById(req: Request, res: Response) {
  ...
  try {
    ...
    if (!individualUtil) {
      return res
        .status(404)
        .json({ error: "Utility was not found, please try again" });
    }
    return res
      .status(200)
      .json(individualUtil ?? { error: `Utility with ${id} was not found.` });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: "There was a server error" });
  }
