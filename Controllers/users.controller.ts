...
async function addUser(req: Request, res: Response) {
  try {
    ...
    const userUtilities = await Promise.all(
      utils.map(async (i) => {
        return await Reference.create({ id: i._id, quantity: 0 });
      })
    );

    const userDecorations = await Promise.all(
      decorations.map(async (i) => {
        return await Reference.create({ id: i._id, quantity: 0 });
      })
    );

    const userFish = await Promise.all(
      allFish.map(async (i) => {
        return await Reference.create({ id: i._id, quantity: 0 });
      })
    );

    const userTanks = await Promise.all(
      tanks.map(async (i) => {
        return await Reference.create({ id: i._id, quantity: 0 });
      })
    );
    ...
    if (!existingUser) {
      ...
      return res
        .status(200)
        .json({ user: savedUser, materials: savedMaterial, status: true });
    } else {
      return res
        .status(409)
        .json({ error: "User already exists on database", status: true });
    }
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({ error: "There was a server error", status: true });
  }
}
...
async function getUserMaterials(req: Request, res: Response) {
  try {
    ...
    const userMaterials = await userMats
      .find({ id: userId })
      .populate({
        path: "fish",
        populate: {
    ...
    const returnData = userMaterials.map((i) => {
      return {
        _id: i._id,
        fish: i.fish.map((j: any) => {
          return {
            _id: j.id._id,
            name: j.id.name,
            imagePath: j.id.imagePath,
            quantity: j.quantity,
          };
        }),
        decorations: i.decorations.map((j: any) => {
          return {
            _id: j.id._id,
            name: j.id.name,
            imagePath: j.id.imagePath,
        ...
        utilities: i.utilities.map((j: any) => {
          return {
            _id: j.id._id,
            name: j.id.name,
            imagePath: j.id.imagePath,
        ...
        tanks: i.tanks.map((j: any) => {
          return {
            _id: j.id._id,
            name: j.id.Litres + " L tank",
            imagePath: j.id.imagePath,
            quantity: j.quantity,
          };
        }),
...
async function getUserMaterial(req: Request, res: Response) {
  try {
    ...
    switch (category) {
      case "fish":
        userMaterial = await userMats.aggregate([
          ...
          {
            $match: {
              [`${category}.id`]: new mongoose.Types.ObjectId(item as string),
            },
          },
          {
            $project: {
      ...
      case "utilities":
        ...
        foundInBuilds = await preBuilds
          .find({ "utilities.id": item })
          .populate({
            path: "MainFish",
            select: "imagePath",
            model: FishModel,
    ...
    if (!userMaterial) {
      return res
        .status(404)
        .json({ msg: `user with ${userId} does not exist` });
    }
    ...
    switch (category) {
      ...
      default:
        return res
          .status(200)
          .json({ mat: userMaterial[0], builds: foundInBuilds });
    }
  } catch (error) {
    console.log(error);
...
async function getQuestions(req: Request, res: Response) {
  try {
    ...
    const userQuery = await Users.find({ email: Email }).select([
      "securityQuestions",
    ]);
    ...
    if (!userQuery) {
      return res
        .status(409)
        .json({ msg: "User does not exist on database", status: false });
    }
    const question = random(userQuery[0].securityQuestions);
    console.log(userQuery);
    return res
      .status(200)
      .json({ user: userQuery[0]._id, question: question, status: true });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: "There was a server error" });
  }
...
async function udpateQuantity(req: Request, res: Response) {
  try {
    ...
    if (!userMaterial) {
      return res
        .status(404)
        .send({ msg: `Usermaterials with userId ${userId} was not found` });
    }
