...
async function viewUserBuild(req: Request, res: Response){
  try {
    ...
    // console.log('-------------------------');
    const userBuild = await userBuilds.findOne({userId: userId, _id: buildId}).populate({
      path: "MainFish",
      select: "imagePath",
      model: FishModel,
    })
    ...
    const userBuild2 = await userBuilds.findOne({userId: userId, _id: buildId});
    ...
    if(userBuild2){
      ...
      userMaterial.fish.map((i: any) =>{
        filteredUserMaterial.push({ id: i.id.toString(), quantity: i.quantity });
      });
    ...
    if(!userBuild || !filter){
      return res.status(404).json({msg: `Users build with ID ${buildId} was not found`})
    }
...
export default {getAllUserBuilds, viewUserBuild};